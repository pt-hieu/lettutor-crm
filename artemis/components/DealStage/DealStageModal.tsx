...
import { DealStageData, DealStageTable } from './DealStageTable'
...
export const DealStageModal = ({ visible, handleClose }: Props) => {
  ...
  const [dataSource, setDataSource] = useState<DealStageData[]>([])
...
function isValidData(data: DealStageData) {
  return !!data.name && !(data.isDeleted && data.isNew)
}

function formatData(data: DealStageData) {
  const newData = { ...data }
